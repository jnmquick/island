...
func createWaveCircle() -> SKShader {
    
    let uniforms: [SKUniform] = [
        SKUniform(name: "u_speed", float: 0.5),
        SKUniform(name: "u_brightness", float: 0.5),
        SKUniform(name: "u_strength", float: 1.0),
        SKUniform(name: "u_density", float: 50.0),
        SKUniform(name: "u_center", point: CGPoint(x: 0.5, y: 0.5)),
        SKUniform(name: "u_color", color: waterBlue)
    ]
    
    return SKShader(fromFile: "wave", uniforms: uniforms)
}
