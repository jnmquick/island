...
class OrangeUnits: SKSpriteNode {
    ...
    func createOrangeUnitLabels(nodeTouched: SKNode) {
        ...
        if let orangeUnit = logicForLightOrangeSquares.orangeUnitArray.first(where: { $0.name == nodeTouched.name }) {
    ...
    func createOrangeBullets() -> SKSpriteNode {
        var orangeBullet: SKSpriteNode
        orangeBullet = SKSpriteNode(imageNamed: "Bullet")
        orangeBullet.setScale(0.15)
        orangeBullet.position = lastOrangeUnit.position
        orangeBullet.zPosition = 1
        orangeBullet.zRotation = lastOrangeUnit.zRotation
        orangeBullet.name = "bullet"
        orangeBullet.physicsBody = SKPhysicsBody(rectangleOf: orangeBullet.frame.size)
...
//            self.orangeUnitShadow.removeAllChildren()
    ...
    func moveOrange(location: CGPoint, unit: SKSpriteNode, view: SKView) {
        ...
        if let orangeUnit = logicForLightOrangeSquares.orangeUnitArray.first(where: { $0.name == unit.name }) {
